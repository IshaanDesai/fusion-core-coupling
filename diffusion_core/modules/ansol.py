...
class Ansol:
    def __init__(self, config, mesh):
        ...
        self._ums = config.get_ums()  # 2nd (radial mode num) zero of Bessel function of first kind of order m,
    ...
    def ansol(self, r, theta, t):
        return math.sin(self._m * theta) * special.jv(self._m, self._ums * r) * math.exp(-math.pow(self._ums, 2) * t)

    def ansol_gradient(self, r, theta, t):
        return math.sin(self._m * theta) * math.exp(-math.pow(self._ums, 2) * t) * self._ums * (1 / 2) * \
    ...
    def compare_ansoln(self, u, t, logger_ansoln):
        ...
        for i in range(self._ntheta):
            for j in range(self._nrho):
                del2 += self._drho * self._dtheta * self._rho[j] * math.pow(
                    u[i, j] - self.ansol(self._rho[j], self._theta[i], t), 2)
