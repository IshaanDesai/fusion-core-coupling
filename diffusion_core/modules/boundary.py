...
class Boundary:
        ...
        for i in range(1, 3):
            ...
            field[ip[i], j] = 4 * field[ip[i], j - 1] / 3 - field[ip[i], j - 2] / 3 - (
                    self._drho * self._g_rt[ip[i], j]) / (3 * self._dtheta * self._g_rr[ip[i], j]) * (
                                  ...
                                  ip[i + 1], j - 2] -
                                      field[ip[i - 1], j - 2]) + (2 * self._drho) / (
                                      3 * math.sqrt(self._g_rr[ip[i], j])) * (flux[ip[i]])

        for i in range(1, self._ntheta - 1):
            # Dirichlet condition at inner boundary
            field[i, 0] = ansol.ansol(self._rho[0], self._theta[i], t)
            # Neumann condition at outer boundary (2nd order)
            field[i, j] = 4 * field[i, j - 1] / 3 - field[i, j - 2] / 3 - (self._drho * self._g_rt[i, j]) / (
                    3 * self._dtheta * self._g_rr[i, j]) * (2 * field[i - 1, j - 1] - 2 * field[i + 1, j - 1] +
    ...
    def set_bnd_vals_ansol(self, ansol, field, t):
        ...
        for i in range(1, self._ntheta - 1):
            ...
            field[i, j] = 4 * field[i, j - 1] / 3 - field[i, j - 2] / 3 - (self._drho * self._g_rt[i, j]) / (
                    3 * self._dtheta * self._g_rr[i, j]) * (
                                  2 * field[i - 1, j - 1] - 2 * field[i + 1, j - 1] + field[i + 1, j - 2] -
                                  field[i - 1, j - 2]) + (2 * self._drho) / (3 * math.sqrt(self._g_rr[i, j])) * flux
